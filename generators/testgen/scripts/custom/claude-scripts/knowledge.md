...
- **cp_custom_vfp_flags**: 100% across rv32/rv64 and SEW16/32/64. Root cause for rv32+SEW64 0% was script-level `if sew > common.xlen: return`, which prevented any test generation (`cp_asm_count=0`, `std_vec=0`, all crosses 0%). For vector FP flag coverpoints, do not apply the generic sew>xlen skip.
