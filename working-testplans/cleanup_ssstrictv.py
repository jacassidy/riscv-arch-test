...
def main():
    ...
    for i, row in enumerate(rows):
        ...
        if name == "cp_ssstrict_ls_nf<nf>_eew<eew>":
            ...
            row["Feature Description"] = "Run segment load/store instructions with NFIELDS and EEW combinations where NFIELDS * EEW exceeds the implementation limit, across all segment load/store instruction variants"
            row["Expectation"] = "trap"
            row["Spec"] = "The EMUL setting must be such that EMUL * NFIELDS ≤ 8, otherwise the instruction encoding is reserved."
            print(f"  [FILLED] row {i}: {name}")

        elif name == "cp_ssstrictv_ls_eew_2x_elen":
            if not get_desc(row):
                row["Feature Description"] = "Run vector load/store instructions with EEW encoded in the width field set to 2 * ELEN (e.g., if ELEN=64, attempt EEW=128 encoding)"
            if not get_expectation(row):
                row["Expectation"] = "trap"
            if not get_spec(row):
                row["Spec"] = "Vector loads and stores have an EEW encoded directly in the instruction. If the EEW is not a supported width, the instruction encoding is reserved."
            print(f"  [FILLED] row {i}: {name}")

        elif name == "cp_ssstrictv_ls_emul_16":
            if not get_desc(row):
                row["Feature Description"] = "Run vector load/store instructions with SEW/LMUL/EEW combinations producing (EEW/SEW)*LMUL = 16 (e.g., EEW=64, SEW=8, LMUL=2)"
            if not get_expectation(row):
                row["Expectation"] = "trap"
            if not get_spec(row):
                row["Spec"] = "If the EMUL would be out of range (EMUL>8 or EMUL<1/8), the instruction encoding is reserved."
            print(f"  [FILLED] row {i}: {name}")

        elif name == "cp_ssstrictv_ls_emul_nfields_16":
            if not get_desc(row):
                row["Feature Description"] = "Run segment load/store instructions with EMUL and NFIELDS combinations where EMUL * NFIELDS > 8 (e.g., EMUL=2, NFIELDS=5)"
            if not get_expectation(row):
                row["Expectation"] = "trap"
            if not get_spec(row):
                row["Spec"] = "The EMUL setting must be such that EMUL * NFIELDS ≤ 8, otherwise the instruction encoding is reserved."
            ...
            row["Feature Description"] = "Execute a vector load/store instruction with EEW (encoded in width field) smaller than the minimum supported SEW (SEWMIN). For example, vle8.v when SEWMIN > 8"
            row["Expectation"] = "trap"
            row["Spec"] = "Vector loads and stores have an EEW encoded directly in the instruction. If the EEW is not a supported width, the instruction encoding is reserved."
            print(f"  [FILLED] row {i}: {name}")
    ...
    for i, row in enumerate(rows):
        ...
        if name == "csrops_reserved_vcsrs":
            ...
            row["Feature Description"] = "Attempt to read/write CSR addresses 0x00B, 0x00C, 0x00D, 0x00E (reserved for future vector CSRs) and confirm each raises an illegal instruction trap"
            row["Expectation"] = "trap"
            ...
            row["Feature Description"] = "Run vsetvli or vsetivli with vsew field set to 3'b100, 3'b101, 3'b110, 3'b111 (reserved encodings) and verify vill is set to 1"
            ...
            row["Feature Description"] = "Run vsetvli with LMUL and SEW combinations where LMUL < minSEW/ELEN (the ratio is unsupported), and verify vill is set"
            row["Expectation"] = "vill set"
            ...
            row["Feature Description"] = "Run a vector load or store instruction with mew=1 (bit 28 of instruction) and confirm the encoding is reserved"
            row["Expectation"] = "trap"
            row["Bins"] = "1 bin"
